...
class TestCAMUS:
    ...
    def test_getitem(self):
        ...
        assert item["sequence_2CH"] == str(os.path.join("patient0001", "patient0001_2CH_sequence.mhd"))
        assert item["sequence_4CH"] == str(os.path.join("patient0001", "patient0001_4CH_sequence.mhd"))
...
class TestRSNABoneAgeChallenge:
    def setup_class(self):
        ...
        self.dataset_train = RSNABoneAgeChallenge(self.base_path, training=True)

        self.dataset_test = RSNABoneAgeChallenge(self.base_path, training=False)
    ...
    def test_getitem(self):
        for item in self.dataset_train:
            assert item["image"] == os.path.join(
                "boneage-training-dataset", "boneage-training-dataset", "12345.png"
            ) or item["image"] == os.path.join("boneage-training-dataset", "boneage-training-dataset", "12346.png")
        ...
        for item in self.dataset_test:
            assert item["image"] == os.path.join("boneage-test-dataset", "boneage-test-dataset", "11111.png") or item[
                "image"
            ] == os.path.join("boneage-test-dataset", "boneage-test-dataset", "22222.png")
...
class TestRSNAIntracranialHemorrhageDetection:
    def setup_class(self):
        ...
        self.dataset_train = RSNAIntracranialHemorrhageDetection(self.base_path, training=True)

        self.dataset_test = RSNAIntracranialHemorrhageDetection(self.base_path, training=False)
    ...
    def test_getitem(self):
        for item in self.dataset_train:
            assert item["image"] == os.path.join("stage_2_train", "ID_000012eaf.dcm") or item["image"] == os.path.join(
                "stage_2_train", "ID_000039fa0.dcm"
            )
        ...
        for item in self.dataset_test:
            assert item["image"] == os.path.join("stage_2_test", "ID_000000e27.dcm") or item["image"] == os.path.join(
                "stage_2_test", "ID_000009146.dcm"
            )
...
class TestEMIDEC:
    ...
    def test_training_dataset(self):
        dataset = EMIDEC(data_dir=self.path, training=True, transform=None)

        assert len(dataset) == 2

        element = dataset[0]

        image_paths = [self.paths[0], self.paths[2]]

        label_paths = [self.paths[1], self.paths[3]]
    ...
    def test_test_dataset(self):
        dataset = EMIDEC(data_dir=self.path, training=False, transform=None)
...
class TestBrats2020:
    def setup_class(self):
        ...
        self.expected_survivalinfo_keys = [
            "Brats20ID",
            "Age",
            "Survival_days",
            "Extent_of_Resection",
